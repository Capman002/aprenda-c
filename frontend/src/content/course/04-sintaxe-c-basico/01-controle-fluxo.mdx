...
> Conforme _Linguagem C Completa e Descomplicada_ (André Backes): "Os comandos de controle condicional e de repetição completam a apresentação da estrutura básica da linguagem."
...
<Callout type="error" title="Erro Clássico">
  Não confunda `=` (atribuição) com `==` (comparação)! ```c if (x = 5) //
  ERRADO: Isso ATRIBUI 5 a x, não compara! if (x == 5) // CERTO: Isso COMPARA x
  com 5 ```
</Callout>
...
<Callout type="warning" title="O break é essencial!">
  Sem o `break`, a execução "cai" para o próximo `case`: ```c case 'a':
