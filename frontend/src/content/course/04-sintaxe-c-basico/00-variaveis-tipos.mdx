...
> Conforme _Linguagem C Completa e Descomplicada_ (André Backes): "Uma variável deve ser sempre declarada antes de ser usada dentro do programa."
...
<Callout type="warning" title="Atenção">
  Em C, você **deve** declarar o tipo da variável. Diferente de Python ou
  JavaScript, C é uma linguagem **estaticamente tipada**.
</Callout>
...
<Callout type="error" title="Divisão entre Inteiros">
  ...
  menos um operando deve ser `float` ou `double`: ```c float resultado = 5.0 /
  3.0; // 1.666667 ```
</Callout>
